...
document.addEventListener('click', function(e) {
    // 如果点击的元素不在右键菜单（#rightMenu）内
    if (!document.getElementById('rightMenu').contains(e.target)) {
        rm.hideRightMenu();  // 隐藏右键菜单
    }
});
...
function addRightMenuClickEvent() {
    ...
    $("#menu-search").on("click", rm.search),
    ...
    $("#menu-tags").on("click", (function() {
        rm.hideRightMenu()
	})),

    $("#menu-folder-open").on("click", (function() {
        rm.hideRightMenu()
	})),

    $("#menu-copyright").on("click", (function() {
        rm.hideRightMenu()
	}))
...
window.oncontextmenu = function(e) {
	if (document.body.clientWidth > 768) {
        ...
        return o.show(),
        globalEvent = e,
        selectTextNow && window.getSelection() ? (b = !0, c.show(),  d.show(), s.show(), p.hide()) : (c.hide(), d.hide(), s.hide()),
...
rm.searchBaidu = function() {
	btf.snackbarShow("即将跳转到百度搜索", !1, 1000), setTimeout((function() {
...
rm.search = function() {
	btf.snackbarShow("正在站内搜索", !1, 100), setTimeout((function() {
        /**调用进行搜索 */
        rm.hideRightMenu()
        const input = document.querySelector('#local-search-input input')
        // 手动触发 input 事件
        let event = new Event('input');  // 创建一个新的 'input' 事件
         //进行粘贴
        input.value =  selectTextNow
